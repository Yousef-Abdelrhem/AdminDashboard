<template>
    <div class="pr-2 pl-2">
        ...
        <section class="w-full rounded-xl border-2 border-[#EFEFEF] p-4 pt-5 pr-4 pb-5">
            ...
            <div v-if="customers.length === 0" class="text-center text-gray-500 py-4">
                Oops! We couldn't find any customers that match your filters.
            </div>
            <CustomerTable v-else :customers="customers.length ? customers : []"
                v-model:selectedCustomers="selectedCustomers" :currentPage="currentPage" :itemsPerPage="itemsPerPage"
                :openMenuIndex="openMenuIndex" :paginatedCustomers="paginatedCustomers" :totalPages="totalPages"
                :handleAction="handleAction" :toggleMenu="toggleMenu" :prevPage="prevPage" :nextPage="nextPage"
...
function handleFilters(filters) {
    ...
    const filtered = allCustomers.value.filter(customer => {
        ...
        if (filters.idEnabled && filters.id !== '') {
            isValid = isValid && customer.customerId.toString().includes(filters.id.toString());
        }

        if (filters.nameEnabled && filters.name !== '') {
            isValid = isValid && customer.customerName.toLowerCase().includes(filters.name.toLowerCase());
        }

        if (filters.numOrderEnabled && filters.numOrder !== '') {
            isValid = isValid && customer.numberOfOrders.toString().includes(filters.numOrder.toString());
        }
        ...
        if (filters.tagEnabled && filters.tag) {
            isValid = isValid && customer.tags.some(tag => tag.toLowerCase() === filters.tag.toLowerCase());
        }
...
function handleAction(action, customer) {
    ...
    if (action === 'view') {
        ...
        allCustomers.value = allCustomers.value.filter(c => c.customerId !== customer.customerId);
        customers.value = customers.value.filter(c => c.customerId !== customer.customerId);
    }
