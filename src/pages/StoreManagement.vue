...
<template>
  ...
  <div class="overflow-auto rounded-lg border border-gray-200">
    <table class="w-full table-auto">
      ...
      <tbody>
        ...
        >
          ...
          <td class="py-3 text-center text-sm text-[#3D3D3D] sm:text-lg">
            <label class="relative inline-flex cursor-pointer items-center text-[#3D3D3D]">
              <input
                type="checkbox"
                v-model="method.active"
                class="peer sr-only"
              />
              <div class="h-6 w-11 rounded-full bg-gray-300 transition-all peer-checked:bg-[#24B443]"></div>
              <div class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform peer-checked:translate-x-full"></div>
            </label>
          </td>
          <td class="relative py-3 text-center text-sm sm:text-lg">
            <button
              @click="openDropdown(index)"
              class="font-bold text-[#763A26] cursor-pointer"
            >
              ⋮
            </button>
            <div
    ...
    <div class="mb-8 rounded-xl bg-white p-6 shadow">
  <div class="mb-4 flex items-center justify-between">
    <h2 class="text-brown-800 text-xl font-bold">Manage User Roles and Permissions</h2>
    <button
      @click="openModal()"
    ...
    >
      <svg width="20" height="20" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9.5 6V12M12.5 9H6.5" stroke="#763A26" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M9.5 16.5C13.6421 16.5 17 13.1421 17 9C17 4.85786 13.6421 1.5 9.5 1.5C5.35786 1.5 2 4.85786 2 9C2 13.1421 5.35786 16.5 9.5 16.5Z" stroke="#763A26" stroke-width="1.5"/>
      </svg>
      User
    </button>
  </div>
  ...
  <div class="overflow-auto rounded-lg border border-gray-200">
    <table class="w-full">
      ...
      <tbody>
        <tr v-for="(user, index) in users" :key="index" :class="{ 'bg-[#F9D4AD]': index % 2 === 0 }">
          <td class="py-3 text-center text-sm text-[#3D3D3D] sm:text-lg">{{ user.name }}</td>
          <td class="py-3 text-center text-sm text-[#3D3D3D] sm:text-lg">{{ user.email }}</td>

          <td class="h-12 flex items-center justify-center text-center">
            <select
              v-model="user.role"
          ...
          <td class="py-3 text-center text-sm text-[#3D3D3D] sm:text-lg">
            <label class="relative inline-flex cursor-pointer items-center">
              <input type="checkbox" v-model="user.active" class="peer sr-only"/>
              <div class="h-6 w-11 rounded-full bg-gray-300 transition-all peer-checked:bg-[#24B443]"></div>
              <div class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform peer-checked:translate-x-full"></div>
            </label>
          </td>
          ...
          <td class="flex items-center justify-center py-3">
            <button @click="deleteUser(index)" class="text-[#B0B0B0] hover:text-[#763A26]">
              <svg class="icon-path" width="20" height="20" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M14.625 4.125L14.1602 11.6438C14.0414 13.5648 13.9821 14.5253 13.5006 15.2159C13.2625 15.5573 12.956 15.8455 12.6005 16.062C11.8816 16.5 10.9192 16.5 8.99452 16.5C7.06734 16.5 6.10372 16.5 5.38429 16.0612C5.0286 15.8443 4.722 15.5556 4.48401 15.2136C4.00266 14.5219 3.94459 13.5601 3.82846 11.6364L3.375 4.125" stroke="#454545" stroke-width="1.5" stroke-linecap="round"/>
                <path d="M2.25 4.125H15.75M12.0418 4.125L11.5298 3.0688C11.1897 2.3672 11.0196 2.01639 10.7263 1.79761C10.6612 1.74908 10.5923 1.7059 10.5203 1.66852C10.1954 1.5 9.80558 1.5 9.02588 1.5C8.2266 1.5 7.827 1.5 7.49676 1.67559C7.42357 1.71451 7.35373 1.75943 7.28797 1.80988C6.99123 2.03753 6.82547 2.40116 6.49396 3.12844L6.03969 4.125" stroke="#454545" stroke-width="1.5" stroke-linecap="round"/>
                <path d="M7.125 12.375V7.875M10.875 12.375V7.875" stroke="#454545" stroke-width="1.5" stroke-linecap="round"/>
              </svg>
            </button>
          </td>
        </tr>
    ...
    <button class="w-12 h-12 flex justify-center items-center rounded border-2 border-[#F1D6B7]" 
    ...
    <span v-for="(page, index) in getVisiblePages()" :key="index" 
          class="w-12 h-12 flex justify-center items-center rounded border-2 border-[#F1D6B7] cursor-pointer" 
          :class="{ 'bg-[#F1D6B7] text-main-900 font-bold': currentPage === page }" 
    ...
    >
      <div class="relative w-[500px] rounded-lg bg-white p-4 shadow">
        ...
        <div class="flex flex-col gap-3 space-y-3">
          ...
          <div>
            <label class="mb-1 block text-lg text-[#763A26]">User Password</label>
            <input
