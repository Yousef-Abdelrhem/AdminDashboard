  ...
  <div class="overflow-auto rounded-lg border border-gray-200">
    <div v-if="orderStore.orders && orderStore.orders.length > 0">
        ...
        <tbody>
          <tr v-for="(order, index) in paginatedOrders" :key="order.orderId"
            ...
            <td class="py-3 text-center">
              <input type="checkbox" :value="order.orderId" :checked="props.selectedProducts.includes(order.orderId)"
                @change="handleCheckboxChange(order.orderId)" class="accent-main-900 mr-2" />
              {{ order.orderId }}
            </td>
            <td class="py-3 text-center">
            ...
            <td class="px-2 py-3 text-center">
              <div class="relative inline-block">
                <button @click="toggleStatusDropdown(index)" :class="[
      'flex items-center justify-center rounded-full px-4 py-1 text-sm font-medium',
      order.status === 'pending' ? 'bg-amber-300' :
        order.status === 'processing' ? 'bg-teal-200' :
          order.status === 'shipped' ? 'bg-purple-300' :
            order.status === 'delivered' ? 'bg-green-300' :
              order.status === 'canceled' ? 'bg-red-300' : 'bg-gray-200'
    ]">
                  {{ order.status }} ▼
                </button>
                <div v-if="statusDropdownIndex === index"
                  class="absolute z-20 mt-1 w-full rounded-md bg-white shadow-lg">
                  <div class="py-1">
                    <button v-for="status in orderStatuses" :key="status" @click="updateOrderStatus(order, status)"
                      :class="[
      'block w-full px-4 py-2 text-left text-sm',
      status === 'pending' ? 'bg-amber-300' :
        status === 'processing' ? 'bg-teal-200' :
          status === 'shipped' ? 'bg-purple-300' :
            status === 'delivered' ? 'bg-green-300' :
              status === 'cancelled' ? 'bg-red-300' : 'bg-gray-200'
    ]">
                      {{ status }}
                    </button>
                  </div>
            ...
            <td class="px-2 py-3 text-center">{{ order.totalPrice }}</td>

            <td class="relative px-2 py-3 text-center">
              <a class="cursor-pointer" @click.prevent="props.toggleMenu(index)">...</a>
              <div v-if="props.openMenuIndex === index"
                ...
                <ul class="text-sm">
                  ...
                  <li @click="props.handleAction('view', order)" :class="[
      ...
      props.activeAction === 'view' && order.orderId,
                  ...
                  <hr class="ml-2 w-30 text-gray-400" />
                  <li @click="props.handleAction('delete', order)" :class="[
      ...
      <div class="pagination my-4 flex justify-center gap-1 text-gray-600">
        <button @click="props.prevPage" :disabled="props.currentPage === 1"
        ...
        <span v-for="page in props.totalPages" :key="page" @click="props.changePage(page)" :class="[
      ...
      props.currentPage === page ? 'text-main-900 bg-[#F1D6B7]' : '',
        ...
        <button @click="props.nextPage" :disabled="props.currentPage === props.totalPages"
          class="flex h-9 w-9 items-center justify-center rounded border-2 border-[#F1D6B7]">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="size-4">
...
function toggleStatusDropdown(index) {
  statusDropdownIndex.value = statusDropdownIndex.value === index ? null : index;
}
