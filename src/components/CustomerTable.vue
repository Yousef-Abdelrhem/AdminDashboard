<template>
  <div class="overflow-auto rounded-lg border border-gray-200">
    <div v-if="paginatedCustomers && paginatedCustomers.length > 0">
      <table class="w-full">
        <thead>
          <tr class="text-gray-800">
            <th class="px-4 py-5 text-sm md:text-lg">Id</th>
            <th class="px-4 py-5 text-sm md:text-lg">Customer Image</th>
            <th class="px-4 py-5 text-sm md:text-lg">Customer Name</th>
            <th class="px-4 py-5 text-sm md:text-lg">Customer Email</th>
            <th class="px-4 py-5 text-sm md:text-lg">No.of Order</th>
            <th class="px-4 py-5 text-sm md:text-lg">Total</th>
            <th class="px-4 py-5 text-sm md:text-lg">Tags</th>
            <th class="px-4 py-5 text-sm md:text-lg">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            ...
            <td class="px-5 py-3 text-center">{{ customer.numberOfOrders }}</td>
            <td class="px-5 py-3 text-center">${{ orderStore.totalPrice }}</td>
                ...
                <ul class="text-sm">
                  ...
                  <li
                    ...
                    :class="[
                      ...
                      activeAction === 'delete' &&
                      activeCustomerId === product.customerId
                        ? 'text-main-900'
                        : '',
                    ]"
                  >
      ...
      <div class="pagination my-4 flex justify-center gap-1 text-gray-600">
        ...
        <span
          ...
          :class="{ 'text-main-900 bg-[#F1D6B7]': currentPage === page }"
        >
          {{ page }}
        </span>
        <button
    ...
    <div v-else>
      <p class="py-4 text-center text-gray-500">No products available.</p>
    </div>
  </div>
</template>
...
const toggleTags = (customer) => {
  ...
  openTagsForCustomer.value[customerId] =
    !openTagsForCustomer.value[customerId];
};
